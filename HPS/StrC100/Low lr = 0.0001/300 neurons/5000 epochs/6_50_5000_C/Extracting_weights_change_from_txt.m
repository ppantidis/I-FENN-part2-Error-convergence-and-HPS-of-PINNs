...
for i = 1:length(layers)
    for j = 1:length(neurons)
        for k = 1:length(epochs)
            for l = 1:length(runs)
                % Load file
                Weights_Change_StrC100_6_50_5000_lowlr(l,:) = load(['Model11_TestE_StrC100_Weights_Change_', num2str(layers(i)), 'hidlay_', num2str(neurons(j)), 'units_', num2str(epochs(k)), 'ep_inc_164_v', num2str(runs(l)), '.txt']);
            end        
        end
    end
end
...
save Weights_Change_StrC100_6_50_5000_lowlr Weights_Change_StrC100_6_50_5000_lowlr
