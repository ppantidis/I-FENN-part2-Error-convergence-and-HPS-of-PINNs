...
for i = 1:length(layers)
    for j = 1:length(neurons)
        for k = 1:length(epochs)
            for l = 1:length(runs)
                % Load file
                current_file = load(['Model11_TestE_StrF100_History_', num2str(layers(i)), 'hidlay_', num2str(neurons(j)), 'units_', num2str(epochs(k)), 'ep_inc_164_v', num2str(runs(l)), '.txt']);
                % Store cost values
                J_StrF100_12_12_10000_highlr(l,:) = [current_file(epochs(k)) current_file(end)];
            end        
        end
    end
end
...
save Histories_StrF100_12_12_10000_highlr J_StrF100_12_12_10000_highlr
